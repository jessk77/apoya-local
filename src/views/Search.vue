         ...
         <div class="q-pa-md row items-start">
            <div class="col-6 col-sm-12 col-xs-12 col-md-6">
               <q-input
                  ...
                  hint="Busca por nombre o categoría"
               >
               ...
               <q-list padding style="max-height: 500px; overflow-y: scroll;">
                  <q-separator spaced />
                  <div
                     v-for="item in negociosFiltered"
                     v-bind:key="item.uid"
                     ...
                     <q-item class="q-mt-md q-mb-md">
                        ...
                        <q-item-section v-on:click="redirect(item.username)">
                           ...
                           <q-item-label caption
                              >{{ item.description | cutcad }}.</q-item-label
                           >
                        </q-item-section>
                        ...
                        <q-item-section bottom side>
                           <div class="q-gutter-xs">
                              <q-btn
                                 ...
                                 target="_blank"
                                 :href="
                                    `https://api.whatsapp.com/send?phone=521${item.cellphone}&text=Hola!%20Te%20encontre%20en%20apoya%20local%20y%20quiero%20informacion%20por%20favor`
                                 "
                                 size="12px"
            ...
            <div
               v-if="$q.screen.gt.xs"
               class="col-6 col-sm-12 col-xs-12 col-md-6"
            >
               <GmapMap
                  ref="themap"
                  :center="{
...
export default {
   ...
   mounted: async function() {
      ...
      const listadoNegocios = snapshot.docs.map((doc) => doc.data());
      self.negocios = listadoNegocios;
      self.negociosFiltered = listadoNegocios;
   },
   watch: {
      busqueda: function() {
         this.search();
   ...
   methods: {
      search: function() {
         ...
         this.zoom = 16;
         this.negociosFiltered = this.negocios.filter((n) => {
            if (n.name.toUpperCase().includes(this.busqueda.toUpperCase()))
               return n;
            if (n.category.toUpperCase().includes(this.busqueda.toUpperCase()))
               return n;
            if (n.description.toUpperCase().includes(this.busqueda.toUpperCase()))
               return n;
         });

         console.log(this.negociosFiltered);
      },
      test: function() {
         //console.log("end");
      ...
      mouseOver: function(item) {
         ...
         if (item.lat != "") {
            ...
            this.hoverName =
               item.name.toUpperCase() + " - No hay ubicación registrada";
            this.infoWindowPos = {
