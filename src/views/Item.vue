  ...
  <q-layout>
    ...
    <q-page-container class="bg-grey-3">
      <div class="text-center bg-white q-pb-md">
        <q-img style="height: 300px; width: 70%" :src="file_url" :ratio="16/9" />
            ...
            <q-card-actions vertical>
              ...
              <q-btn color="indigo">
                <ShareNetwork
                  ...
                  :title="'Mira todo lo que '+name+' tiene en apoya local!'"
        ...
        <div class="col-7 col-sm-12 col-xs-12 col-md-7 q-pa-sm">
          <q-card class="my-card">
            ...
            <q-tab-panels v-model="tab" animated>
              ...
              <q-tab-panel name="posts" style="max-height: 700px; overflow: scroll">
                <q-btn
                  v-if="sessUid!=='' && uid===sessUid"
                ...
                <div v-for="(value, key) in posts" :key="key">
                  <Post :title="name" :post="value" />
                </div>
                <!-- <template v-slot:loading>
                    <div class="row justify-center q-my-md">
                      <q-spinner-dots color="green-14" size="40px" />
                    </div>
                  </template>
                </q-infinite-scroll>-->
                <div class="q-pt-none" v-if="posts.length===0">No hay Publicaciones</div>
              </q-tab-panel>
            </q-tab-panels>
          </q-card>
        </div>
      ...
      <q-dialog v-model="dialog">
        <q-card>
          ...
          <q-card-section class="q-pt-none">
            <FormPost :uid="sessUid" :user="name" @submited="onPostSubmit" />
          </q-card-section>
        </q-card>
      </q-dialog>
    </q-page-container>
...
export default {
  ...
  mounted: async function() {
    ...
    await this.axios
      .post(
        servidor + "index.php/Inicio/get_data_user",
        JSON.stringify({
      ...
      .then(function(response) {
        ...
        if (response.data.image != "" && response.data.image != null) {
          self.file_url = response.data.image;
        } else {
          self.file_url = "/images/pic_box_2.jpg";
        }
        self.uid = response.data.uid;
        document.title = response.data.name + " | Apoya local";
      });
